// Package logger configures the process-wide zerolog logger for the orchestrator.
package logger
...
// New builds a zerolog.Logger (console in dev, JSON otherwise).
func New(level string, pretty bool, service string) zerolog.Logger {
	...
	return lw.With().Timestamp().Str("service", service).Logger()
}
