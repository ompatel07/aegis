...
## 10. SBOM generation (Phase 2E Task 3)

Every scan now produces a downloadable **Software Bill of Materials** in both
standard formats — **CycloneDX 1.7** and **SPDX 2.3** — generated by Trivy from the
authenticated checkout at scan time, stored per-scan, and exported from the
dashboard (buttons alongside SARIF). Each lists components, versions, and known
CVEs per component (licenses where the ecosystem provides them).

**Verified end-to-end** (full scan of Flask → download via the API):

| Check | Result |
| --- | --- |
| CycloneDX valid (`bomFormat`, `specVersion`) | ✅ **CycloneDX 1.7**, 34 components, 13 CVEs |
| SPDX valid (`spdxVersion`, `dataLicense`) | ✅ **SPDX-2.3**, 35 packages |
| Stored per-scan + served with attachment headers | ✅ `aegis-<scan>.cdx.json` / `.spdx.json` |
| CVEs are real | ✅ 5/5 sampled (CVE-2023-46136, CVE-2024-22195/34064/34069/49766) confirmed in NVD |
| Ownership-gated | ✅ non-owner → HTTP 404 |
| Generation never fails the scan | ✅ best-effort (logged, non-fatal) |

**Honest limitations.** SBOM completeness depends on the repo committing a
**lockfile**: requirements.txt (Flask → 34 components) and go.sum (gin → 37) resolve
fully, but a repo with only a bare `package.json` and no `package-lock.json`
(Express, juice-shop) yields **0 components** — Trivy correctly refuses to
fabricate a dependency tree it cannot resolve. **Dependency license data is
best-effort**: on source-only clones most components come back `NOASSERTION`
because per-package license metadata lives in the installed packages, not the
manifest. Both are inherent to source-scan SBOM tooling, not Aegis-specific. The
optional *license-risk flagging* was therefore **deliberately not shipped** — on
source scans it would be near-empty, and shipping a hollow feature violates the
project's honesty discipline; it becomes worthwhile once dependency resolution
(installed packages) is in scope.
